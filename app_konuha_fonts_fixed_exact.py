...
# Exact font files uploaded by the user.
# The CSS family names below are intentionally the normal family names
# (Cairo / Readex Pro / Inter), while the files are loaded locally.
...
INTER_FONTS = {
    ...
    400: font_file("Inter_18pt-Regular.ttf") or first_glob("Inter-VariableFont*.ttf"),
...
# If the remaining Inter weights are not uploaded, the variable font can
# safely provide the missing weights.
...
if INTER_VARIABLE:
    LOCAL_FONT_CSS += font_face_css("Inter", INTER_VARIABLE, "100 900")


CAIRO_REGULAR = CAIRO_FONTS.get(400)
CAIRO_BOLD = CAIRO_FONTS.get(700) or CAIRO_REGULAR
...
def draw_rtl(draw, position, value, font, fill):
    value = str(value or "")
    try:
        draw.text(position, value, font=font, fill=fill, anchor="ra")
    except Exception:
