...
def _font_with_raqm(path, size):
    if not path or not path.exists() or not _raqm_available():
        return None
    try:
...
def get_export_font(size, bold=False):
    candidates = [CAIRO_BOLD if bold else CAIRO_REGULAR]
    candidates += ([READEX_FONTS.get(700), TAJAWAL_FONTS.get(700), TAJAWAL_FONTS.get(800)]
                   if bold else [READEX_FONTS.get(400), TAJAWAL_FONTS.get(400)])
    candidates += [
    ...
    for path in candidates:
        ...
        if font is not None:
            return font
        font = _font_plain(path, size)
        if font is not None:
            return font
    return ImageFont.load_default()


def _shape_rtl_fallback(value):
    if not (arabic_reshaper and bidi_get_display):
        return str(value)
    try:
        return bidi_get_display(arabic_reshaper.reshape(str(value)))
    except Exception:
        return str(value)
...
def draw_rtl(draw, position, value, font, fill, anchor="ra"):
    ...
    if _raqm_available():
        try:
            ...
            pass
    rendered = _shape_rtl_fallback(value) if has_arabic else value
    try:
...
def make_pdf_from_png(png_bytes):
    ...
    image_stream = io.BytesIO(bytes(png_bytes))
    image_stream.seek(0)
...
__LOCAL_FONT_CSS__
    ...
    /* KONUHA swipe navigation */
    [data-testid="stRadio"] { width:100% !important; margin:4px 0 18px !important; }
    [data-testid="stRadio"] > div { width:100% !important; }
    [data-testid="stRadio"] div[role="radiogroup"] {
        display:flex !important; flex-direction:row !important; flex-wrap:nowrap !important;
        align-items:center !important; gap:10px !important; overflow-x:auto !important; overflow-y:hidden !important;
        width:100% !important; max-width:100% !important; box-sizing:border-box !important;
        padding:9px !important; scrollbar-width:none !important; -webkit-overflow-scrolling:touch !important;
        background:rgba(15,18,32,.82) !important; border:1px solid #272b3d !important; border-radius:18px !important;
    }
    ...
    [data-testid="stRadio"] div[role="radiogroup"] > label {
        ...
        min-height:42px !important; padding:9px 16px !important; margin:0 !important;
        border:1px solid #303448 !important; border-radius:13px !important;
        background:linear-gradient(180deg,#151928,#10131f) !important; color:#e9eaf0 !important;
        white-space:nowrap !important; writing-mode:horizontal-tb !important;
    }
    [data-testid="stRadio"] div[role="radiogroup"] > label > div:first-child { display:none !important; }
    [data-testid="stRadio"] div[role="radiogroup"] > label p,
    [data-testid="stRadio"] div[role="radiogroup"] > label span {
        white-space:nowrap !important; writing-mode:horizontal-tb !important; display:block !important;
        margin:0 !important; line-height:1.2 !important;
    }
    [data-testid="stRadio"] div[role="radiogroup"] > label:hover { border-color:#7654cf !important; transform:translateY(-1px) !important; }
    [data-testid="stRadio"] div[role="radiogroup"] > label[data-checked="true"] {
        background:linear-gradient(135deg,rgba(139,92,246,.38),rgba(217,70,239,.24)) !important;
        border-color:#8b5cf6 !important; box-shadow:0 7px 25px rgba(139,92,246,.18) !important;
    }

    /* Compact horizontal KONUHA navigation */
    div[role="radiogroup"] {
    ...
    div[role="radiogroup"] > label:hover {
        border-color: #7654cf !important;
        transform: translateY(-1px);
    }

    div[role="radiogroup"] > label[data-checked="true"] {
        background: linear-gradient(135deg, rgba(139,92,246,.34), rgba(217,70,239,.22)) !important;
        border-color: #8b5cf6 !important;
        box-shadow: 0 6px 24px rgba(139,92,246,.16);
    }
    ...
    .k-home-showcase {
        ...
        background:
            radial-gradient(420px 180px at 85% 15%, rgba(139,92,246,.14), transparent 70%),
            radial-gradient(360px 180px at 10% 90%, rgba(217,70,239,.10), transparent 70%),
            linear-gradient(145deg, rgba(22,25,40,.94), rgba(11,13,22,.96));
        box-shadow: 0 18px 50px rgba(0,0,0,.18);
    }

    .k-home-showcase::after {
        content: "";
        position: absolute;
        width: 220px;
        height: 220px;
    ...
    .k-showcase-title {
        ...
        font-weight: 850;
        background: linear-gradient(90deg, #fff, #c7b8ff, #ee82df);
        -webkit-background-clip: text;
        background-clip: text;
        color: transparent;
    }
    ...
    .k-feature-icon {
        ...
        display: grid;
        place-items: center;
        background: linear-gradient(135deg, rgba(139,92,246,.20), rgba(217,70,239,.14));
    ...
    .k-section-info {
        position:relative; overflow:hidden; margin:14px 0 24px; padding:22px; border-radius:22px;
        border:1px solid #2b2f42;
        background:radial-gradient(380px 150px at 88% 15%,rgba(139,92,246,.13),transparent 72%),
                   radial-gradient(300px 150px at 12% 90%,rgba(217,70,239,.08),transparent 72%),
                   linear-gradient(145deg,rgba(20,23,36,.96),rgba(10,12,20,.98));
        box-shadow:0 16px 44px rgba(0,0,0,.14);
    }
    .k-section-top { display:flex; align-items:flex-start; justify-content:space-between; gap:16px; margin-bottom:16px; }
    .k-section-title { margin-top:4px; color:#f6f2ff; font-size:clamp(20px,2.5vw,29px); font-weight:850; }
    .k-section-sub { margin-top:6px; color:#9ca2b3; font-size:13px; line-height:1.8; max-width:760px; }
    .k-section-badge { flex:0 0 auto; padding:8px 12px; border-radius:999px; border:1px solid rgba(139,92,246,.34); background:rgba(139,92,246,.10); color:#ded7f7; font-size:11px; font-weight:800; }
    .k-info-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:10px; }
    .k-info-item { display:flex; gap:10px; align-items:flex-start; min-height:74px; padding:13px; border-radius:16px; border:1px solid #272b3c; background:rgba(8,10,17,.50); }
    .k-info-emoji { width:34px; height:34px; display:grid; place-items:center; flex:0 0 auto; border-radius:11px; border:1px solid rgba(139,92,246,.22); background:linear-gradient(135deg,rgba(139,92,246,.18),rgba(217,70,239,.12)); font-size:17px; }
    .k-info-title { color:#f0eef5; font-size:12px; font-weight:800; margin-bottom:3px; }
    .k-info-text { color:#9298aa; font-size:11px; line-height:1.65; }
...
# =========================================================
    ...
    section_info("قسم التصدير", "حوّل بيانات KONUHA إلى ملفات جاهزة للحفظ والمشاركة.", [("🖼️", "PNG", "تقرير بصري منسق ومناسب للمشاركة."), ("📄", "PDF", "نسخة PDF من نفس التقرير المصور."), ("📊", "Excel / CSV", "بيانات خام قابلة للفتح والتحليل.")])
    ...
    if df.empty:
        ...
        st.caption("تقرير بصري مرتب وجاهز للمشاركة بصيغة PNG أو PDF.")
