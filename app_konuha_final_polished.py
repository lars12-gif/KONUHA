...
def _font_with_raqm(path, size):
    """Load a font with Pillow's RAQM engine for real Arabic shaping + bidi."""
...
def get_export_font(size, bold=False):
    ...
    raise RuntimeError(
        "تعذر تحميل خط عربي مع دعم RAQM. تأكد من وجود مجلد fonts/ "
        "وأن نسخة Pillow تدعم RAQM."
    )


def draw_rtl(draw, position, value, font, fill, anchor="ra"):
    """Render Arabic/mixed text directly through Pillow's RAQM engine."""
    value = str(value or "")
    if not value:
        return
    ...
    if _raqm_available():
        kwargs["direction"] = "rtl" if has_arabic else "ltr"
        kwargs["language"] = "ar" if has_arabic else "en"
    else:
        raise RuntimeError("دعم RAQM غير متوفر في بيئة التصدير.")

    draw.text(position, value, **kwargs)
