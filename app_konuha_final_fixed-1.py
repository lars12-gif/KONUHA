...
# Arabic shaping for PNG/PDF exports. Streamlit UI is unaffected.
try:
...
def _shape_arabic(value):
    """Return visually ordered, joined Arabic text for Pillow exports."""
    value = str(value or "")
    if not value:
        return value
...
def draw_rtl(draw, position, value, font, fill, anchor="ra"):
    """Draw Arabic/mixed text correctly in generated PNG reports."""
    value = str(value or "")
    shaped = _shape_arabic(value)
    # Once bidi shaping is applied, Pillow should treat the string as visual
    # LTR text. For environments where the helper packages are unavailable,
    # Pillow's RAQM path remains the secondary fallback.
    if shaped != value:
        try:
            # get_display() returns visual-order text, so use a normal anchor.
            visual_anchor = anchor
            draw.text(position, shaped, font=font, fill=fill, anchor=visual_anchor)
            return
        except Exception:
            pass
    try:
        draw.text(position, value, font=font, fill=fill, anchor=anchor, direction="rtl")
    except Exception:
        try:
